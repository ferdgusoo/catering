        ...
        <div>
            <div class="overflow-auto">
                <div class="p-6 text-gray-900">
                    <form action="/menu/{{ $menu->id }}" method="POST" class="bg-white rounded-xl flex flex-row gap-20 p-12" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="flex flex-col gap-20 w-1/2">
                            <div>
                                <label for="name" class="text-sm font-medium text-gray-700">Nama Menu</label>
                                <input value="{{ $menu->name }}" type="text" name="name" class="border-gray-300 focus:border-black focus:ring-black rounded-md shadow-sm w-full">
                            </div>
                            <div>
                                <label for="package" class="text-sm font-medium text-gray-700">Isi Menu</label>
                                <input value="{{ $menu->package }}" type="text" id="package" name="package" class="border-gray-300 focus:border-black focus:ring-black rounded-md shadow-sm w-full">
                            </div>
                            <div>
                                <label for="price" class="text-sm font-medium text-gray-700">Harga Menu</label>
                                <input value="{{ $menu->price }}" type="number" id="price" name="price" class="border-gray-300 focus:border-black focus:ring-black rounded-md shadow-sm w-full">
                            </div>
                        ...
                        <div class="flex flex-col justify-between gap-20 w-1/2">
                            <div>
                                ...
                                <textarea name="desc" rows="5" class="border-gray-300 focus:border-black focus:ring-black rounded-md shadow-sm w-full">{{ $menu->desc }}</textarea>
                            </div>
                            <div>
